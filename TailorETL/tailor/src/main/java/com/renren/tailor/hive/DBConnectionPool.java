...
public class DBConnectionPool {
	...
	private void createConnections(int numConnections) {
		...
		for (int x = 0; x < numConnections; x++) {
			...
			try {
				...
				log.error("fail to create db connection!" + e.getMessage());
				e.printStackTrace();
			}
			log.info("success to create db connection ......");
		}
	}
			...
			if (!pConn.isBusy()) {
				...
				if (!testConnection(conn)) {
					...
					try {
						...
						log.error("fail to create db connection! "
								+ e.getMessage());
						return null;
					}
	...
	private void closeConnection(Connection conn) {
		try {
			...
			log.error("fail to close connection.The error msg is ： "
					+ e.getMessage());
		}
	}
